...
<div class="content-container">
    ...
    @endsection
    @include('keluar._modal-keluar-tambah')
...
{!! Form::close() !!}
    <script>
        ...
        var url = '{{ route("gismobile.keluar.dt", [base64_encode(\Carbon\Carbon::now()->format("Y-m-d")),\
        ...
        $("#display-btn").on("click", function(){
                ...
                var url1 = "{{ route('gismobile.keluar.dt', ['param1','param2' ,'param3','param4','param5']) }}"
        ...
        function hapus(no_kirim){
            var urldel = "{{ route('gismobile.keluar.delete', ['param1']) }}"
    ...
    $("#kd_trans").change(function(){
        ...
        function gen_trans() {
            ...
            url = '{{ route('gismobile.keluar.gen_trans') }}'
            token = window.Laravel.csrfToken
        ...
        function gen_qty() {
            ...
            var kd_trans = $("#kd_trans").val();
            console.log(no_kirim)
            url = '{{ route('gismobile.keluar.gen_qty') }}'
            token = window.Laravel.csrfToken
    ...
    $('#sub-btn').on('click', function() {
        ...
        function inputData(){
            swal({
                ...
                showLoading()
                $.ajax({
                    url: "{{ route('gismobile.keluar.store') }}",
                    type: 'POST',
        ...
        function print_excel() {
            ...
            var produk = $('#produk').val();
            var id_customer = $('#id_customer').val();
            var url = '{{ route("gismobile.keluar.print", ['param1', 'param2', 'param3', 'param4', 'param5']) }}'
