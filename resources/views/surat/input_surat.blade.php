...
{!! Form::open(['url' => route('gismobile.surat.store'), 'method' => 'post', 'files'=>'true', 'id'=>'form_id']) !!}
<div class="content-container">
        ...
        <div class="box-body form-horizontal">
            <div class="row form-group">
                <div class="col-sm-12">
                <div class="col-sm-3">
                            {!! Form::label('lblkode_brg', 'No Surat') !!}
                            {!! Form::text('no_surat', null, ['class'=>'form-control','placeholder' => 'Otomatis', 'name' => 'no_surat', 
                            'id' => 'no_surat', 'readonly'=>'true']) !!}
                        </div>

                        <div class="col-sm-3">
                            {!! Form::label('lblkode_brg', 'No PO') !!}
                            {!! Form::text('no_po', null, ['class'=>'form-control','placeholder' => 'Otomatis', 'name' => 'no_po', 
                            'id' => 'no_po', 'readonly'=>'true']) !!}
                        </div>

						<div class="col-sm-3">
                            {!! Form::label('lblkode_brg', 'No Kendaraan') !!}
                            {!! Form::text('kendaraan', null, ['class'=>'form-control','placeholder' => 'Nomor', 'name' => 'kendaraan', 
                            'id' => 'kendaraan']) !!}
                        </div>
...
{!! Form::close() !!}
    ...
    @include('surat._modal-surat-tambah')
    @endsection
@section('scripts')
...
$('#btn-add-data').on('click', function() {
	...
	function getIdkirim(id){
		...
		var row = $('#table-lhp tbody tr').length;
			...
			var url = '{{ route("gismobile.surat.modal") }}'
...
$('#form_scpag').submit(function (e, params) {
	...
	if (!localParams.send) {
		...
		swal({
			...
			showLoading()
			$.ajax({
				url: "{{ route('gismobile.surat.store') }}",
				type: 'post',
